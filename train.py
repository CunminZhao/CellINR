...
def train(cfg):
    ...
    while cfg.i_step <= cfg.max_iter:
        for batch in cfg.trainloader:
            kk=kk+1
            cfg.optim.zero_grad()
            gts, coords, coords_full, shape, scale = batch
            shape = tuple(t.item() for t in shape)
            scale = scale.item()
            gts_raw, gts_full, gts_hessian_full = gts
            
            gts_raw, coords, gts_full, gts_hessian_full = gts_raw.squeeze(0), coords.squeeze(0), gts_full.squeeze(0), gts_hessian_full.squeeze(0)
            rgb, rgb0 = cfg.Render(coords, is_train=True)
            loss1 = cfg.loss_fn(rgb, rgb0, gts_raw)
            if torch.isinf(loss1).item() or loss1 == 0:
                print(f"Skipping step {cfg.i_step} due to loss1 being inf / 0")
                continue
            ...
            with torch.no_grad():
                cfg.Update(loss, rgb.cpu().numpy(), gts_raw.cpu().numpy())
                if cfg.i_step % cfg.log_iter == 0: 
                    cfg.Log()

                if cfg.i_step % cfg.save_iter == 0: 
                    cfg.Save()
...
args = argparse.Namespace(expname='CellINR_test', cfg='./configs/example.yaml', scale=1, 
                          mode='train', file='./data/test.nii.gz', 
